...
- ✅ Opacity support (applied to fill and stroke alpha)
- ✅ Transform support via matrix stack (translate, scale)
...
```cpp
void draw();                                   // Draw at origin
void draw(float x, float y);                   // Draw at position
void draw(float x, float y, float w, float h); // Draw at position with size
```
...
- Translation and scaling via matrix stack

🔄 TODO:
- Gradients (linear, radial)
- SVG transform attributes (parsed by nanosvg but not applied)
- Text elements
- Clipping paths
