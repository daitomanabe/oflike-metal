        ...
        for (NSVGshape* shape = image->shapes; shape != nullptr; shape = shape->next) {
            ...
            for (NSVGpath* nsvgPath = shape->paths; nsvgPath != nullptr; nsvgPath = nsvgPath->next) {
                ...
                if (opacity < 1.0f) {
                    ...
                    if (path.isFilled()) {
                        uint8_t r, g, b, a;
                        path.getFillColor(r, g, b, a);
                        a = static_cast<uint8_t>(a * opacity);
                        path.setFillColor(r, g, b, a);
                    }
                    // Modify stroke alpha
                    if (path.getStrokeWidth() > 0.0f) {
                        uint8_t r, g, b, a;
                        path.getStrokeColor(r, g, b, a);
                        a = static_cast<uint8_t>(a * opacity);
                        path.setStrokeColor(r, g, b, a);
                    }
                }
...
void ofxSvg::draw(float x, float y, float width, float height) {
    ...
    // Calculate scale factors based on SVG dimensions
    float scaleX = (impl_->width > 0.0f) ? width / impl_->width : 1.0f;
    float scaleY = (impl_->height > 0.0f) ? height / impl_->height : 1.0f;
