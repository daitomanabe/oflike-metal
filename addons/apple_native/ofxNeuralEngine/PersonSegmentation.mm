...
SegmentationResult PersonSegmentation::segment(ofTexture& texture) {
    ...
    // DEFERRED: Direct Metal processing optimization
    //
    // Rationale:
    // - VNImageRequestHandler expects CVPixelBuffer/CGImage/CIImage input
    // - readToPixels() provides necessary format conversion
    // - Direct MTLTexture processing would require custom Vision request handler
    // - Current path maintains Vision framework compatibility
    //
